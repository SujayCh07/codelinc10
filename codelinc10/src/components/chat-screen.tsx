...
export function ChatScreen({
  ...
  resources = [],
  resourceHeading = "Learning links",
}: ChatScreenProps) {
    ...
    <div className="flex min-h-screen flex-col bg-[#F7F4F2] text-[#2A1A1A]">
      ...
      <main className="flex-1 overflow-hidden">
        <div className="mx-auto flex h-full w-full max-w-2xl flex-col">
          {(resources.length > 0 || quickPrompts.length > 0) && (
            <section className="space-y-4 px-4 pt-4">
              {resources.length > 0 && (
                <div className="rounded-3xl border border-[#F0E6E7] bg-white p-4 shadow-sm">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.35em] text-[#7F1527]">{resourceHeading}</p>
                  <div className="mt-3 space-y-2">
                    {resources.map((item) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="group flex items-start gap-3 rounded-2xl border border-[#E2D5D7] bg-[#FBF7F6] p-3 text-left transition hover:border-[#A41E34] hover:bg-white"
                      >
                        <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-white text-[#A41E34] shadow-sm">
                          <ExternalLink className="h-4 w-4" />
                        </span>
                        <span className="text-sm leading-relaxed text-[#4D3B3B]">
                          <span className="block font-semibold text-[#2A1A1A]">{item.title}</span>
                          <span className="text-xs text-[#7F1527]">{item.description}</span>
                        </span>
                      </a>
                    ))}
                  </div>
