...
import { saveChatSession } from "@/lib/db-client"
...
export function ChatModal({
  ...
  const persistSession = useCallback(
    async (history?: Msg[]) => {
      try {
        ...
        if (typeof window !== "undefined" && !(window as any).__lifelens_user_id) {
          ;(window as any).__lifelens_user_id = user_id
        }

        const session_history = (history ?? messagesRef.current)
          .filter((entry) => entry.role === "user" || entry.role === "assistant")
          .map((entry) => ({ role: entry.role, content: entry.content, ts: entry.ts }))

        if (!session_history.length) return
        ...
        setSessionError(String((error as Error)?.message ?? error))
      }
    },
    [sid, userId]
  )
  ...
  async function send(textParam?: string, extraContext?: Record<string, unknown>) {
    ...
    if (!textParam) {
      const userMessage: Msg = { id: crypto.randomUUID(), role: "user", ts: Date.now(), content: text }
    ...
    try {
      ...
      if ("error" in payload) {
        setMessages((m) => [
          ...m,
          { id: crypto.randomUUID(), role: "error", ts: Date.now(), content: `Error: ${payload.error}` },
        ])
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        {sessionError && (
          <div className="px-5 pb-2 text-xs text-red-600">Chat history not saved: {sessionError}</div>
        )}
