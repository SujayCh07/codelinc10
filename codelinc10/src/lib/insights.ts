...
export function buildInsights(enrollment: EnrollmentFormData): FinMateInsights {
  ...
  if (timeline.length < 3) {
    timeline.push(
      ...
      {
        period: "Next 30 days",
        title: "Meet with a benefits coach",
        description: "Schedule a FinMate session to keep your plan aligned.",
      },
      {
        period: "This quarter",
        title: "Automate contributions",
        description: "Lock in savings transfers and enrollment reminders.",
      },
    )
  }
  ...
  return {
    ...
    statement: topBenefitTitles.length
      ? `Based on your answers, FinMate recommends tackling ${formatBenefitList(topBenefitTitles)} next.`
      : "FinMate analyzed your profile and mapped benefit picks that fit your answers.",
